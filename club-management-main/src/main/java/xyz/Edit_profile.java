...
public class Edit_profile extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            RequestDispatcher rd = request.getRequestDispatcher("update_profile.jsp");
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse response)
        ...
        try {
            ...
            Connection con = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe", "system", "Dhanada@2002");
            Statement smt = con.createStatement();
            String qry = "UPDATE student_table SET name='" + name + "', sic='" + sic + "', gender='" + gender
                    + "', branch='" + branch + "', year_of_gradn='" + yog + "', ph_no='" + phno + "', password='" + pass + "' WHERE email='" + email + "'";
            int i = smt.executeUpdate(qry);
            if (i > 0) {
                session.setAttribute("msg", "1 record updated");
                req.getRequestDispatcher("student_pg.jsp").forward(req, response);
            } else {
                session.setAttribute("msg", "Error updating record");
            }
        } catch (Exception ee) {
            ee.printStackTrace();
        }
